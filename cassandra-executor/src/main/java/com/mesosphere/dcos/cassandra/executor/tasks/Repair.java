...
/**
 * Implements anti-entropy, primary range, sequential repair by executing RepairTask by delegating repair to the
 * CassandraDaemonProcess.
 */
public class Repair implements ExecutorTask {
    ...
    private void repairKeyspace(String keyspace, List<String> columnFamilies) throws Exception {
        ...
        sendStatus(driver, Protos.TaskState.TASK_RUNNING, String
                        .format("Completed repair : keySpace = %s, columnFamilies = %s", keyspace, columnFamilies));
    }
    ...
    public void run() {
        try {
            ...
            LOGGER.error("Repair failed Keyspace not valid:", e);
            sendStatus(driver, Protos.TaskState.TASK_FAILED, e.getMessage());
        } catch (IOException e) {
            LOGGER.error("Repair failed Keyspace not valid:", e);
            sendStatus(driver, Protos.TaskState.TASK_FAILED, e.getMessage());
        } catch (IllegalArgumentException e) {
            LOGGER.error("Repair failed Table not part of keyspace:", e);
            sendStatus(driver, Protos.TaskState.TASK_FAILED, e.getMessage());
        } catch (Throwable t) {
